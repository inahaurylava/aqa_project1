...
class Bouquet:
    ...
    def contains_flower(self, flower_name: str) -> bool:
        ...
        return any(f == flower_name for f in list(map(lambda fl: fl.name, self.flowers)))

    def __str__(self):
        return "Состав букета: " + ", ".join(list(map(lambda fl: fl.name, self.flowers))) + " . Аксессуары: " + ", ".join(list(map(lambda acc: acc.name, self.accessories)))
